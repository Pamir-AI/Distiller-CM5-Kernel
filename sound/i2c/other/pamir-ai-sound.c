...
/**
 * struct pamir_ai_sound_data - private data for pamir AI sound
 * @client: I2C client
 * @dev: device structure
 * @volume: volume level (0-100)
 * @input_gain: input gain level (0-100)
 */
struct pamir_ai_sound_data {
...
/**
 * pamir_ai_sound_set_volume - set the volume of the AIC3204 device
 ...
 */
static int pamir_ai_sound_set_volume(struct pamir_ai_sound_data *data,
				     u8 volume)
{
	int ret;
	u8 hp_val, dac_val;
...
/**
 * pamir_ai_sound_set_input_gain - set the input gain of the AIC3204 device
 * @data: private data structure
 ...
 * Return: 0 on success, negative error code on failure
 */
static int pamir_ai_sound_set_input_gain(struct pamir_ai_sound_data *data,
					 u8 gain)
{
	int ret;
	u8 adc_val;
...
/**
 * pamir_ai_sound_get_volume - get the current volume of the AIC3204 device
 * @data: private data structure
 *
 ...
 * Return: 0 on success, negative error code on failure
 */
static int pamir_ai_sound_get_volume(struct pamir_ai_sound_data *data)
{
	int ret;
	u8 hp_val, dac_val;
	u8 volume = 0;
...
/**
 * pamir_ai_sound_get_input_gain - get the current input gain of the AIC3204 device
 * @data: private data structure
 *
 ...
 * Return: 0 on success, negative error code on failure
 */
static int pamir_ai_sound_get_input_gain(struct pamir_ai_sound_data *data)
{
	int ret;
	u8 adc_val;
	u8 gain;
				 ...
				 struct device_attribute *attr, char *buf)
{
	...
	ret = pamir_ai_sound_get_volume(data);
	if (ret < 0)
		return ret;
				  ...
				  const char *buf, size_t count)
{
	...
	ret = pamir_ai_sound_set_volume(data, volume);
	if (ret < 0)
		return ret;
			       ...
			       struct device_attribute *attr, char *buf)
{
	...
	ret = pamir_ai_sound_get_input_gain(data);
	if (ret < 0)
		return ret;
				...
				size_t count)
{
	...
	ret = pamir_ai_sound_set_input_gain(data, gain);
	if (ret < 0)
		return ret;
...
static const struct attribute_group pamir_ai_sound_attr_group = {
	.attrs = pamir_ai_sound_attrs,
};

static int pamir_ai_sound_probe(struct i2c_client *client)
{
	struct pamir_ai_sound_data *data;
	int i, ret;
	...
	ret = sysfs_create_group(&client->dev.kobj, &pamir_ai_sound_attr_group);
	if (ret < 0) {
	...
	ret = pamir_ai_sound_set_volume(data, data->volume);
	if (ret < 0) {
		dev_err(&client->dev, "Failed to set initial volume: %d\n",
			ret);
		sysfs_remove_group(&client->dev.kobj,
				   &pamir_ai_sound_attr_group);
		return ret;
	}

	ret = pamir_ai_sound_set_input_gain(data, data->input_gain);
	if (ret < 0) {
...
static void pamir_ai_sound_remove(struct i2c_client *client)
{
	struct pamir_ai_sound_data *data = i2c_get_clientdata(client);
...
static const struct i2c_device_id pamir_ai_sound_id[] = { { "pamir-ai-sound",
							    0 },
							  {} };
MODULE_DEVICE_TABLE(i2c, pamir_ai_sound_id);

static const struct of_device_id pamir_ai_sound_of_match[] = {
	{ .compatible = "pamir-ai,sound" },
	{}
};
MODULE_DEVICE_TABLE(of, pamir_ai_sound_of_match);

static struct i2c_driver pamir_ai_sound_driver = {
	.driver = {
		.name = "pamir-ai-sound",
		.of_match_table = pamir_ai_sound_of_match,
	},
	.probe = pamir_ai_sound_probe,
	.remove = pamir_ai_sound_remove,
	.id_table = pamir_ai_sound_id,
};

module_i2c_driver(pamir_ai_sound_driver);
